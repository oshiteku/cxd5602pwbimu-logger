...
mod tests {
    ...
    fn test_parse_sensor_data_valid() {
        ...
        assert!((data.temp - 10.0).abs() < f32::EPSILON, "Temperature should be 10.0");
    ...
    fn test_bit_conversion() {
        ...
        assert!((result.temp - 5.0).abs() < f32::EPSILON, "40A00000 should be 5.0");
        assert!((result.gx - 3.0).abs() < f32::EPSILON, "40400000 should be 3.0");
        assert!((result.gy - (-2.0)).abs() < f32::EPSILON, "C0000000 should be -2.0");
        assert!((result.gz - 0.0).abs() < f32::EPSILON, "00000000 should be 0.0");
        assert!((result.ax - 1.0).abs() < f32::EPSILON, "3F800000 should be 1.0");
        assert!((result.ay - (-1.0)).abs() < f32::EPSILON, "BF800000 should be -1.0");
        assert!((result.az - (-0.0)).abs() < f32::EPSILON, "80000000 should be -0.0");
    }
}